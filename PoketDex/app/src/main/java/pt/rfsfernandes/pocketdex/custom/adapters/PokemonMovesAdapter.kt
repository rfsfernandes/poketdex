...
class PokemonMovesAdapter(private val context: Context, private val mMovesItemClick: MovesItemClick) : RecyclerView.Adapter<MovesViewHolder>() {
    ...
    private fun populateItemRows(holder: MovesViewHolder, position: Int) {
        ...
        var chosenFlavour = ""
        val country = Locale.getDefault().language.split("-").toTypedArray()[0].lowercase(Locale.getDefault())
        ...
        holder.textViewMoveEffect.text = item?.effectEntry?.effect?.replace("  ", " ")?.replace("\$effect_chance%", item.effectChance.toString())
        ...
        holder.textViewMovePower.setOnClickListener { e: View? -> mMovesItemClick.clickedMovesItem("", item?.type?.urlId, Constants.MOVES_ITEM.TYPE) }
    ...
    inner class MovesViewHolder(val mView: View) : RecyclerView.ViewHolder(mView) {
        val textViewMoveTitle: TextView
        val textViewMoveEffect: TextView
        val textViewMoveShortEffect: TextView
        val textViewMovePower: TextView
        val textViewMoveDescription: TextView
        val selectedView: View
        var mItem: Moves? = null

        init {
            textViewMoveTitle = mView.findViewById(R.id.textViewMoveTitle)
            textViewMoveEffect = mView.findViewById(R.id.textViewMoveEffect)
            textViewMoveShortEffect = mView.findViewById(R.id.textViewMoveShortEffect)
            textViewMovePower = mView.findViewById(R.id.textViewMovePower)
            textViewMoveDescription = mView.findViewById(R.id.textViewMoveDescription)
